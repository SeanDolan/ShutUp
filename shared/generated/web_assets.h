  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 640px) {
      ...
      .grid, .door-grid { grid-template-columns: 1fr; }
      .row > button { flex: 1 1 140px; }
      .sound-table, .sound-table tbody, .sound-table tr,
      .overlay-table, .overlay-table tbody, .overlay-table tr { display: block; width: 100%; }
      .sound-table thead, .overlay-table thead { display: none; }
      .sound-table tr, .overlay-table tr {
      ...
      .sound-table td, .overlay-table td {
        display: grid;
      ...
      .sound-table td::before, .overlay-table td::before {
      ...
      .sound-table td:first-child::before { display: none; }
      .repeat-cell { text-align: left; }
      .repeat-cell input { width: 24px; height: 24px; }
      .sound-table button, .overlay-table button { min-height: 42px; }
      .overlay-table .num-col {
        display: block;
        text-align: left;
        padding: 0 0 10px;
        margin-bottom: 4px;
      ...
      .overlay-table .num-col::before { content: "Door "; color: var(--ink); font-size: 16px; }
      .overlay-table input[type="color"] { width: 100%; height: 42px; padding: 2px; border: 1px solid var(--line); border-radius: 4px; background: #fff; }
    }
  </style>
</head>
<body>
  ...
  <main>
    ...
    <section>
      <h2>Door Sensors</h2>
      <p class="muted">Enable only the normally open reed switch inputs that are physically installed.</p>
      <div class="door-grid">
  ...
  <script>
    ...
    function renderSoundRows(config) {
      ...
      $("soundRows").innerHTML = actions.map((action, index) => `
        <tr>
          <td data-label="Action">${action.name}</td>
          <td data-label="Cab"><select id="action${index}Cab">${soundOptionsHtml(options, action.cab || "None")}</select></td>
          <td data-label="Canopy"><select id="action${index}Canopy">${soundOptionsHtml(options, action.canopy || "None")}</select></td>
          <td data-label="Repeat" class="repeat-cell"><input id="action${index}Repeat" type="checkbox" ${action.repeat ? "checked" : ""}></td>
          <td data-label="Delay"><input id="action${index}Delay" type="number" min="0" step="100" value="${action.delay || 0}"></td>
          <td data-label="Demo"><button type="button" data-demo="${index}">Play</button></td>
        </tr>`).join("");
    ...
    function renderOverlayRows(config) {
      ...
      $("overlayRows").innerHTML = overlays.map((overlay, index) => `
        <tr>
          <td data-label="Door" class="num-col">${index + 1}</td>
          <td data-label="Name"><input id="overlay${index}Name" type="text" maxlength="31" value="${escapeAttr(overlay.name || `Door #${index + 1}`)}"></td>
          <td data-label="Width"><input id="overlay${index}Width" type="number" min="0" max="170" value="${overlay.width || 0}"></td>
          <td data-label="Height"><input id="overlay${index}Height" type="number" min="0" max="320" value="${overlay.height || 0}"></td>
          <td data-label="X"><input id="overlay${index}X" type="number" min="0" max="170" value="${overlay.x || 0}"></td>
